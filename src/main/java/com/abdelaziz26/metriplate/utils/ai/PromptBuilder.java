...
public class PromptBuilder {
    ...
    public String build(Long userId) throws PlanGenerationException {
        ...
        // 5. Filter out ingredients that contain any allergenic nutrient
        List<String> allergenicNutrientNames = allergies.stream()
                .map(a -> a.getNutrient().getName())
                .toList();

        List<Ingredient> safeIngredients = ingredients.stream()
                .filter(ing -> {
                    // an ingredient is unsafe if ANY of its nutrients is in the allergy list
                    return ing.getNutrients().stream()
                            .noneMatch(n -> allergenicNutrientNames.contains(n.getName()));
                })
                .toList();
